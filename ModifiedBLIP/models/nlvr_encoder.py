...
from transformers_swin.activations import ACT2FN
from transformers_swin.file_utils import (
    ModelOutput,
)
from transformers_swin.modeling_outputs import (
...
from transformers_swin.modeling_utils import (
    PreTrainedModel,
    apply_chunking_to_forward,
    find_pruneable_heads_and_indices,
    prune_linear_layer,
)
from transformers_swin.utils import logging
from transformers_swin.models.bert.configuration_bert import BertConfig
...
class BertAttention(nn.Module):
    ...
    def forward(
        ...
        if type(encoder_hidden_states)==list:   
            ...
            outputs = (attention_output,) + self_outputs0[1:]  # add attentions if we outputs them
        else:        
            ...
            outputs = (attention_output,) + self_outputs[1:]  # add attentions if we outputs them
        return outputs
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        if mode=='multimodal':
            ...
            outputs = outputs + cross_attention_outputs[1:-1]  # add cross attentions if we outputs attention weights
...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        # The outputs weights are the same as the input embeddings, but there is
        # an outputs-only bias for each token.
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        encoder_hidden_states  (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, sequence_length, hidden_size)`, `optional`):
            Sequence of hidden-states at the outputs of the last layer of the encoder. Used in the cross-attention if
